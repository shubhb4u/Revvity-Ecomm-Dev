...
import ECOM_GoToCart  from '@salesforce/label/c.ECOM_GoToCart';
import ECOM_Reason  from '@salesforce/label/c.ECOM_Reason';
import ECOM_SuccessCartAddition  from '@salesforce/label/c.ECOM_SuccessCartAddition';
import ECOM_DownloadTemplate  from '@salesforce/label/c.ECOM_DownloadTemplate';
import ECOM_UploadTemplate  from '@salesforce/label/c.ECOM_UploadTemplate';
import ECOM_AddToCart  from '@salesforce/label/c.ECOM_AddToCart';
import ECOM_AddMoreItem  from '@salesforce/label/c.ECOM_AddMoreItem';
...
import ECOM_FailedCartAddition  from '@salesforce/label/c.ECOM_FailedCartAddition';
import ECOM_EnterPartNumberAndQuantities  from '@salesforce/label/c.ECOM_EnterPartNumberAndQuantities';
...
export default class Ecom_quickOrder extends LightningElement {
    ...
    labels = {
        ...
        ECOM_EnterPartNumberAndQuantities,
        ECOM_PartNumber,
        ECOM_Qty,
        ECOM_AddMoreItem,
        ECOM_AddToCart,
        ECOM_UploadTemplate,
        ECOM_DownloadTemplate,
        ECOM_SuccessCartAddition,
        ECOM_Reason,
        ECOM_GoToCart,
        ECOM_UploadDropzoneText, // RWPS-2631
        ECOM_UploadButtonText, // RWPS-2631
        ...
        @track isPunchoutUser;//RWPS-2132 START
        @wire(getRecord, {
            recordId: USER_ID,
        ...
        openModal()
        {
            ...
            // RWPS-3826 START
            setTimeout(() => {
                if(this.refs.mainModalContainer){
                    this.refs.mainModalContainer.focus();
                }
            }, 10);
            // RWPS-3826 END
        }
        ...
        escapeKeyCallback(event) {
            if (event.key === 'Escape' || event.keyCode === 27) {
                if(this.showModal && !this.showUploadModal && !this.showMessagePage) {
        ...
        loadBasedOnDeviceType() {
            if (this.device.isMobile) {
                ...
                this.footerCSS = 'slds-modal__footer';
                ...
                this.footerCSS = 'slds-modal__footer slds-p-right_xx-large';
        ...
        defaultingValues()
        {
            ...
            this.showRemovedPartsMessage=false;
        ...
        // it will return the CSS for disabled Add To Cart Button
        get addToCartButtonCSS()
        {
            return (this.quickOrderRows[0].PartNumber == '' ||this.quickOrderRows[0].Quantity == '') ?'ecom_btn-brand-disabled button-css':'ecom_btn-brand button-css';
            // return this.isAddToCartDisabled?'ecom_btn-brand-disabled slds-m-left_medium':'ecom_btn-brand slds-m-left_medium';
        }

        // logic to enable the Add To Cart Button
        get isAddToCartDisabled()
        {
            return (this.quickOrderRows[0].PartNumber == '' ||this.quickOrderRows[0].Quantity == '') ?true:false;
        }
            ...
            addItemsToCart({
                ...
                if(error?.body && error?.body?.exceptionType==='ConnectApi.ConnectApiException'){
                    ...
                    for(let i in this.cartItems) {
                        ...
                        this.erroneousPartsToDisplay.push({
                            partNumber : this.productIdTopartNumberMap.get(this.cartItems[i]?.Product2Id), //RWPS-3772
                            reason : ECOM_105101
                        });
                    }
                    this.handleNextSteps();
            ...
            for(let i in this.addToCartRows)
            {
                if(!retrievedProducts.has(this.addToCartRows[i].PartNumber.toUpperCase()))
                {
                    ...
                    this.erroneousPartsToDisplay.push({
                        ...
                        reason : ECOM_105101 //RWPS-3631 invalid part number
                    });
        ...
        doValidation() {
            ...
            this.addToCartRows = [];
            //RWPS-275 - garora@rafter.one - 19 March 2024 - Initial check to clear errors on empty inputs
            ...
            this.quickOrderRows.forEach(product => {
                if (!product.PartNumber) {
                    ...
                    //RWPS-275 - garora@rafter.one - 19 March 2024 - check for previous error on related part field to see if it still withholds
                    /**RWPS-275 changes begins */
        ...
        removeFromList(event)
        {
            ...
            for (let orderIndex=0;orderIndex<tempList.length;orderIndex++)
            {
                ...
                if(ind === orderIndex){
                    const iconElement = inputElement.parentElement.parentElement.querySelector('.ecom-qo-deleteitem-btn');
                    if(iconElement){
                        const qoDelIconSrc = iconElement.getAttribute('data-inactive-icon-src');
                        iconElement.setAttribute('src',qoDelIconSrc);
                    }
                }
            }
    ...
    downloadCSVFile() {
        ...
        downloadElement.download = 'Quick Order Data.csv';
    ...
    formatData(data){
        ...
        data.forEach((element, index, array) => {
            keysArray = Object.keys(element);
            let partNumVal = String(element.PartNumber).trim(); //RWPS-3772
    ...
    parse(csv){
      ...
      checkFileForErrors(lines,headers){
        ...
        for(var i in data)
        {
            ...
            if(parsedQty>999)
            {
                this.errorMessage.push('Max quantity allowd for product is 999.Please make sure that all the values are less than the max quantity.');
                break;
            }
        }
      ...
      async processUploadedData(){//RWPS-2132
        ...
        try {
            if(this.isPunchoutUser)
            {
                ...
                const result = await processProductExclusions({communityId: communityId, partNumberList:partnumbers});//RWPS-1456
        ...
        for(var i in data)
        {
            ...
            temp["imgSrc"]=this.images.qodelete;
        ...
        if(partsWithError!='')
        {
            //this.errorMessage.push('There are duplicate Part Numbers : '+partsWithError);
